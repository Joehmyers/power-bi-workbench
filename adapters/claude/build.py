...
It also writes CLAUDE.md at the repo root: a shim that imports AGENTS.md
(the neutral agent instructions), since Claude Code does not read AGENTS.md
natively.

plugins/, .claude-plugin/, and CLAUDE.md are generated output. Do not edit
them by hand; edit the neutral sources and rerun this script. CI fails if
they drift.
...
def main():
    ...
    # Claude Code auto-loads CLAUDE.md but does not read AGENTS.md natively;
    # emit the import shim so the neutral instructions reach it.
    ...
    print(f"Generated {len(mp['groups'])} plugins, .claude-plugin/marketplace.json, and CLAUDE.md")
