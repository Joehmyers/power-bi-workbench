#!/bin/bash
#
# Validate report-to-model binding in definition.pbir.
#
# Usage: validate-report-binding.sh FILE [FILE...]
#
...
# Arguments not named definition.pbir are ignored. Validates that
# definition.pbir has a valid semantic model connection:
...
# Skip if jq not available (needed to inspect definition.pbir itself)
command -v jq &>/dev/null || exit 0

# With several arguments, validate each in its own invocation so the
# straight-line single-file logic below stays simple.
if [[ $# -gt 1 ]]; then
    RC=0
    for FILE_ARG in "$@"; do
        bash "${BASH_SOURCE[0]:-$0}" "$FILE_ARG" || RC=2
    done
    exit $RC
fi
...
# ── Take the file path from the first argument ──────────────────────────────
