...
#
# Usage: pbi-hooks.sh <subcommand> [command-text]
#
# Platform-neutral hook script: works standalone or behind any agent's hook
# mechanism. Adapters translate agent events into this call; see
# adapters/README.md. `command-text` is the shell command the agent ran (or
# is about to run); each subcommand applies its own gating, so passing an
...
#   exit 2 = blocking error (stderr shown to the calling agent)
...
# The shell command that triggered this hook, passed by the caller.
COMMAND_TEXT="${2:-}"

HOOK_DIR="$(cd "$(dirname "${BASH_SOURCE[0]:-$0}")" 2>/dev/null && pwd)" || exit 0
CONFIG_PATH="$HOOK_DIR/config.yaml"
# PROJECT_DIR may be set by the caller; CLAUDE_PROJECT_DIR is kept as a
# fallback for compatibility with Claude Code's hook environment.
...
# Require jq (used to parse the cached model metadata)
